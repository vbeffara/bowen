\section{Théorème de Perron-Frobenius de Ruelle}
  ...
  \begin{theorem}[Théorème de Perron-Frobenius de Ruelle] \label{thm:rpf}
  ...
  \begin{proof}
    ...
    d'après le lemme \ref{lemma:compact}.
    ...
    Or, $\L f(x) \leq B_{m+1}e^{b\alpha^{m+1}}\L f(x')$ (comme vu dans la preuve du lemme \ref{lemma:compact}).
  ...
  \begin{proof}
    ...
    Si $f^{\pm}F \not= 0$, alors grâce aux lemmes \ref{lemma:Cr} et \ref{lemma:expo}, on a
  ...
  \begin{proof}
    ...
    On applique le lemme \ref{lemma:fF} avec $F = 1$, ce qui donne pour $m$ assez grand,
    \begin{align*}
